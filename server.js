...
app.post("/api/register", checkMongoConnection, async (req, res) => {
  try {
    ...
    if (username.length < 3 || username.length > 20) {
      return res
        .status(400)
        .json({ error: "Tên đăng nhập phải từ 3-20 ký tự" });
    }
...
app.put(
  ...
  async (req, res) => {
    try {
      ...
      // Cập nhật display name trong memory nếu user đang online
      const userInfo = users.get(user.username);
      if (userInfo) {
        userInfo.displayName = user.displayName;
        // Broadcast lại danh sách users để các client cập nhật
        broadcastUsers();
      }
...
// Lấy display name của một user
app.get(
  "/api/user/:username/display-name",
  checkMongoConnection,
  async (req, res) => {
    try {
...
// Danh sách user online: username → { socketId, publicKey, displayName }
const users = new Map();
  ...
  for (const [username, info] of users.entries()) {
    ...
    if (!displayName && isMongoConnected()) {
      try {
        ...
        console.error("Lỗi lấy displayName:", error);
      }
    }
    snapshot.push({
      username,
...
io.on("connection", (socket) => {
  ...
  socket.on("join", async ({ username, publicKey, displayName }) => {
    ...
    if (!isMongoConnected()) {
      console.warn(
        "⚠️ MongoDB chưa kết nối, bỏ qua cập nhật user và tin nhắn offline"
      );
  ...
  socket.on(
    "dm",
    async ({ messageId, from, to, encryptedMessage, timestamp }) => {
